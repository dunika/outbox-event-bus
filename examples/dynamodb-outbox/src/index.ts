...
export class DynamoDBOutbox implements IOutbox {
  ...
  private async recoverStuckEvents() {
    ...
    // Find events in PROCESSING that have exceeded timeout
        ...
        for (const item of result.Items) {
            try {
                await this.docClient.send(new UpdateCommand({
                    ...
                    ConditionExpression: "#status = :processing", // Avoid racing with a successful completion
                    ExpressionAttributeNames: { "#status": "status" },
                    ExpressionAttributeValues: {
                        ":pending": "PENDING",
  ...
  private async processBatch(handler: (events: BusEvent[]) => Promise<void>) {
    ...
    // 2. Claim events (concurrency control)
    for (const item of result.Items) {
      try {
        await this.docClient.send(new UpdateCommand({
          TableName: this.tableName,
        ...
        // ConditionalCheckFailedException means someone else claimed it, skip
