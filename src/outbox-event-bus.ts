...
export class OutboxEventBus<TContext = unknown> implements IOutboxEventBus<TContext> {
  private readonly handlers = new Map<string, EventHandler<string, unknown>[]>()
  private maxListeners = 10

  constructor(
    private readonly outbox: IOutbox<TContext>,
    private readonly onMaxListeners: (
      bus: OutboxEventBus<TContext>,
      eventType: string,
      count: number
    ) => void,
  ...
  async emit<T extends string, P>(event: BusEvent<T, P>, context?: TContext): Promise<void> {
    await this.outbox.publish([event], context)
  }

  async emitMany<T extends string, P>(events: BusEvent<T, P>[], context?: TContext): Promise<void> {
  ...
  rawListeners(eventType: string): ((...args: unknown[]) => unknown)[] {
    return [...(this.handlers.get(eventType) ?? [])] as ((...args: unknown[]) => unknown)[]
  }
  ...
  async waitFor<T extends string, P = unknown>(
    ...
    return new Promise((resolve, reject) => {
      ...
      const handler: EventHandler<T, P> = async (event) => {
        cleanup?.()
        resolve(event)
      }
  ...
  async start(): Promise<void> {
    await this.outbox.start(async (events) => {
